import { Story, Preview, Meta, ArgsTable, Source, Description, Canvas  } from "@storybook/addon-svelte-csf";
import dedent from 'ts-dedent';
import Version from '~/version.mdx';
import { Loader } from "@kws3/ui";
import { action } from '@storybook/addon-actions';
import * as LoaderStories from './loader.stories';

export let args={
  spinner_color: "grey",
  spinner_size: "medium",
  background_color: "transparent",
  background_size: "medium",
  class:"",
  style: "",
  is_inline: false,
  has_overlay: false
}

<Version />

<Meta of={LoaderStories} />

<Description
  markdown={dedent`
# Loader
## Graphic to display a loading status to users.
`}
/>

<Canvas>
  <Story of={LoaderStories.Loader} />
</Canvas>

<Controls />

### Usage

##### Import the component:

<Source language="javascript" code={ dedent`import { Loader } from '@kws3/ui';`} />

##### Import the CSS:

<Source language="javascript" code={ dedent`@import '@kws3/ui/styles/Loader.scss';`} />

##### SCSS customization variables with default values:

<Source
  language="css"
  code={ dedent`
    $kws-theme-colors: $colors;
    $kws-loader-width: 2rem;
    $kws-loader-height: 2rem;
    $kws-loader-width-medium: ($kws-loader-width * 1.5);
    $kws-loader-height-medium: ($kws-loader-height * 1.5);
    $kws-loader-width-large: ($kws-loader-width * 3);
    $kws-loader-height-large: ($kws-loader-height * 3);
    $kws-loader-color: #ccc;
    $kws-loader-overlay-background: rgba(0,0,0,0.15);
  `}
/>
