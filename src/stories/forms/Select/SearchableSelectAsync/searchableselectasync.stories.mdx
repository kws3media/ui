...
export let args = {
  ...
  hotFilter: async () => await fetchFromApi(filter),
};
...
<div class="notification is-primary is-light">
  <div class="columns">
    <div class="column">
      <h3>
        Async filter function <code>hotFilter</code>
      </h3>
      <p>
      ...
      <p>
        Note: To improve rendering performance, slice down the options if items array contain a large dataset on a single match.
      </p>
      <Source
        language="jsx"
        code={dedent`<SearchableSelect
      ...other_options
      hotFilter={async ()...... items.map(...).slice(0, 20);}
