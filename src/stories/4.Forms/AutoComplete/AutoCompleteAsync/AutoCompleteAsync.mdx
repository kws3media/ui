import {
  Controls,
  Meta,
  Source,
  Canvas,
} from "@storybook/blocks";
import dedent from "ts-dedent";
import Version from "~/version.mdx";
import * as Stories from "./AutoCompleteAsync.stories";

<Version />

# AutoComplete (Async)
## An Input component that supports auto complete tips.

<Meta of={Stories} />
<Canvas />
<Controls />

<div className="notification is-primary is-light sb-unstyled">
  <div className="columns">
    <div className="column">
      <h3 className="subtitle">
        Input format <code>options</code>
      </h3>
      <p className="">
        Can send either an array of strings, or an array of objects.<br></br>Always
        ensure the shape of the array items are consistent.
      </p>
     <p className="">See supported data formats in example story above.</p>
      <p className="">Note: <code>option.label</code> contains matched characters wrapped inside <code>&lt;span class="h"&gt;</code> tags. </p>
      <p className="">Note: Highlighted results can be turned off by setting <code>highlighted_results</code> property to <code>false</code></p>
    </div>
    <div className="column">
      <h3 className="subtitle">Customising dropdown text items</h3>
      <p className="">See documentation for default slot.</p>
      <h4 className="subtitle">Example:</h4>
      <Source
        language="jsx"
        code={dedent`<AutoComplete
      ...other_options
      let:option>
      <span class="has-text-danger">
        {@html option.label}
      </span>
    </AutoComplete>`}
      />
    </div>
  </div>
</div>

### Usage

#### Import the component:

<Source
  code={dedent`import { AutoComplete } from '@kws3/ui';`}
/>

#### Import the SCSS:
_Default values shown as example_
<Source
  code={dedent`
      @use '@kws3/ui/styles/AutoComplete.scss' with (
        $theme-colors: dv.$colors,
        $radius: cv.getVar("radius"),
        $border-color: cv.getVar("border"),
        $box-shadow: cv.getVar("shadow"),
        $focus-border-color: cv.getVar("input-focus-border-color"),
        $focus-box-shadow-size: cv.getVar("focus-shadow-size"),
        $focus-box-shadow-color: hsl(
          cv.getVar("focus-h"),
          cv.getVar("focus-s"),
          cv.getVar("focus-l"),
          cv.getVar("focus-shadow-alpha")
        ),
        $disabled-background-color: cv.getVar("input-disabled-background-color"),
        $disabled-border-color: cv.getVar("input-disabled-border-color"),
        $disabled-color: cv.getVar("input-disabled-color"),
        $selecting-color: cv.getVar("primary-invert"),
        $selecting-background: cv.getVar("primary"),
        $text-matches-color: currentColor,
        $text-matches-background: transparent,
        $text-matches-font-weight: cv.getVar("weight-bold"),
        $options-background: cv.getVar("scheme-main-ter"),
        $__modal-z: 41,
      );
    `}
/>
