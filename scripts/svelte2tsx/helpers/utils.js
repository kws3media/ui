 ...
 * @returns {string}
 */
function resolve_aliases(input, file, content, aliases) {
  /**
   * @param {string} match
   * @param {string} _
  ...
  const replace_import_path = (match, _, import_path) => {
    for (const [alias, value] of Object.entries(aliases)) {
      ...
      const full_import_path = path.join(
        value,
        import_path.slice(alias.length)
      );
      let resolved = posixify(
        path.relative(path.dirname(full_path), full_import_path)
      );
 ...
 */
function scan(input, extensions) {
