# UNRELEASED
  - Bug Fix `MultiSelect` - output raw value object initially (if given) instead of `value_key` format .
  - Bug Fix `SearchableSelect` and `MultiSelect` (Async) -  add class active on most matched item when options appears in dropdown.
